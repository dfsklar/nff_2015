...
if (seriesTypes.column) {
	...
	seriesTypes.column.prototype.alignDataLabel = function (point, dataLabel, options,  alignTo, isNew) {
		...
		if (dlBox) { // Area range uses this method but not alignTo
			...
			if (inverted) {
				var aTx = chart.plotWidth - alignTo.y - alignTo.height; // << DOES NOT TAKE "left" AND "width" OF AXIS INTO ACCT
        if (this.chart.yAxis.count() > 1) {
          aTx = this.yAxis.left + 15/*this.yAxis.width*/ - alignTo.y - alignTo.height;
        }
				alignTo = {
          //DFSKLAR//
					x: aTx,
